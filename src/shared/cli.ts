// Status of the `research-agent` command-line launcher that the app can install into the user's PATH.
...
export type CliLauncherStatus = {
  ...
  target: string
  // Whether `target`'s directory is on the user's PATH, i.e. `research-agent` is callable by name.
  onPath: boolean
