...
| Unsafe fallback replay    | Ledger-computed request identity, exact ordered targets, two-alternate limit, verified refusal scope, and side-effect-invalidated reservations. |
...
dependency or remote host can be compromised; ad-hoc-signed packages do not provide release
authenticity; and byte-equivalent safety fallback does not guarantee provider consistency. These
risks must be visible rather than described as eliminated.
