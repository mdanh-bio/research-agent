...
describe('dataFolderName', () => {
  ...
  it('is ResearchAgent when packaged', () => {
    appMock.isPackaged = true
    expect(dataFolderName()).toBe('ResearchAgent')
  })

  it('is ResearchAgent-DEV in dev (not packaged)', () => {
    appMock.isPackaged = false
    expect(dataFolderName()).toBe('ResearchAgent-DEV')
  })
})
...
describe('dataRootForParent', () => {
  it('joins the parent with the data folder name', () => {
    appMock.isPackaged = true
    expect(dataRootForParent('/mnt/data')).toBe(join('/mnt/data', 'ResearchAgent'))
  })
})
...
describe('dataRootForPicked', () => {
  ...
  it('is case-sensitive off Windows (a differently-cased folder is not the data folder)', () => {
    ...
    appMock.isPackaged = true
    const lower = '/mnt/data/researchagent'
    expect(dataRootForPicked(lower)).toBe(join(resolve(lower), 'ResearchAgent'))
  })
})
...
describe('samePath / isPathInsideOrEqual (platform-aware)', () => {
  ...
  it('compares case-insensitively on win32 (NTFS is case-insensitive)', () => {
    setPlatform('win32')
    expect(samePath(p('Data', 'ResearchAgent'), p('data', 'researchagent'))).toBe(true)
    expect(isPathInsideOrEqual(p('Data'), p('data', 'ResearchAgent'))).toBe(true)
  })
...
describe('computeDefaultDataRoot', () => {
  ...
  it('defaults to <home>/ResearchAgent for a fresh config root', () => {
    // resolveConfigRoot() resolves under homeDir but nothing has been created there.
    expect(computeDefaultDataRoot()).toBe(join(homeDir, 'ResearchAgent'))
  })

  it('keeps packaged E2E config and data under the disposable certification root', () => {
    const e2eRoot = join(homeDir, 'certification')
    vi.stubEnv('RESEARCH_AGENT_E2E_STORAGE_ROOT', e2eRoot)

    expect(resolveConfigRoot()).toBe(e2eRoot)
    expect(computeDefaultDataRoot()).toBe(join(e2eRoot, 'ResearchAgent'))
  })

  it('stays at the config root when it already has legacy data and no ResearchAgent subdir', async () => {
  ...
  it('does not treat a config root with an ResearchAgent subdir as legacy', async () => {
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    await mkdir(join(configRoot, 'ResearchAgent'), { recursive: true })
  ...
  it('stays at the legacy config root when <home>/ResearchAgent exists but carries a migration marker', async () => {
    ...
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    const homeDefault = join(homeDir, 'ResearchAgent')
  ...
  it('does not treat a markerless partial <home>/ResearchAgent copy as committed', async () => {
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    await mkdir(join(homeDir, 'ResearchAgent', 'artifacts'), { recursive: true })
  ...
  it('treats an explicitly configured <home>/ResearchAgent as the committed default', async () => {
    const configRoot = resolveConfigRoot()
    const homeDefault = join(homeDir, 'ResearchAgent')
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
...
describe('computeDefaultDataRoot (dev mode)', () => {
  ...
  it('stays at the (dev) config root when it already has legacy data and no ResearchAgent-DEV subdir', async () => {
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
  ...
  it('defaults to <home>/ResearchAgent-DEV for a fresh (dev) config root', () => {
    // resolveConfigRoot() resolves under homeDir but nothing has been created there.
    expect(computeDefaultDataRoot()).toBe(join(homeDir, 'ResearchAgent-DEV'))
  })

  it('prefers an explicitly configured <home>/ResearchAgent-DEV over legacy data', async () => {
    ...
    const configRoot = resolveConfigRoot()
    await mkdir(join(configRoot, 'artifacts'), { recursive: true })
    const homeDefault = join(homeDir, 'ResearchAgent-DEV')
    await mkdir(join(homeDefault, 'artifacts'), { recursive: true })
    initDataRoot(homeDefault)
...
describe('resolveStorageRoot', () => {
  ...
  it('uses the normal development directory by default', () => {
    expect(resolveStorageRoot()).toBe(join('/Users/tester', '.research-agent-project'))
  })

  it('uses an absolute development preview override without changing HOME', () => {
    vi.stubEnv('RESEARCH_AGENT_STORAGE_ROOT', '/tmp/research-agent-preview/storage')

    expect(resolveStorageRoot()).toBe(normalize('/tmp/research-agent-preview/storage'))
    expect(appMock.getPath).not.toHaveBeenCalled()
  })

  it('uses an absolute disposable E2E root in packaged builds', () => {
    appMock.isPackaged = true
    vi.stubEnv('RESEARCH_AGENT_E2E_STORAGE_ROOT', '/tmp/research-agent-certification/storage')

    expect(resolveStorageRoot()).toBe(normalize('/tmp/research-agent-certification/storage'))
    expect(appMock.getPath).not.toHaveBeenCalled()
  })

  it('rejects an ambiguous relative E2E root', () => {
    vi.stubEnv('RESEARCH_AGENT_E2E_STORAGE_ROOT', 'certification/storage')

    expect(() => resolveStorageRoot()).toThrow(
      'RESEARCH_AGENT_E2E_STORAGE_ROOT must be an absolute path'
    )
  })

  it('rejects an ambiguous relative preview override', () => {
    vi.stubEnv('RESEARCH_AGENT_STORAGE_ROOT', 'preview/storage')
  ...
  it('ignores the preview override in packaged builds', () => {
    appMock.isPackaged = true
    vi.stubEnv('RESEARCH_AGENT_STORAGE_ROOT', '/tmp/ignored')

    expect(resolveStorageRoot()).toBe(join('/Users/tester', '.research-agent'))
  })

  it('ignores legacy Open Science storage overrides without the compatibility opt-in', () => {
    vi.stubEnv('OPEN_SCIENCE_STORAGE_ROOT', '/tmp/legacy-storage')
    vi.stubEnv('OPEN_SCIENCE_E2E_STORAGE_ROOT', '/tmp/legacy-e2e')

    expect(resolveStorageRoot()).toBe(join('/Users/tester', '.research-agent-project'))
  })

  it('accepts a legacy storage override only with the compatibility opt-in', () => {
    vi.stubEnv('RESEARCH_AGENT_ALLOW_LEGACY_OPEN_SCIENCE_ENV', '1')
    vi.stubEnv('OPEN_SCIENCE_STORAGE_ROOT', '/tmp/legacy-storage')

    expect(resolveStorageRoot()).toBe(normalize('/tmp/legacy-storage'))
  })

  it('accepts a legacy packaged E2E root only with the compatibility opt-in', () => {
    appMock.isPackaged = true
    vi.stubEnv('RESEARCH_AGENT_ALLOW_LEGACY_OPEN_SCIENCE_ENV', '1')
    vi.stubEnv('OPEN_SCIENCE_E2E_STORAGE_ROOT', '/tmp/legacy-e2e')

    expect(resolveStorageRoot()).toBe(normalize('/tmp/legacy-e2e'))
  })

  it('prefers the Research Agent override when legacy compatibility is enabled', () => {
    vi.stubEnv('RESEARCH_AGENT_ALLOW_LEGACY_OPEN_SCIENCE_ENV', '1')
    vi.stubEnv('RESEARCH_AGENT_STORAGE_ROOT', '/tmp/research-agent-storage')
    vi.stubEnv('OPEN_SCIENCE_STORAGE_ROOT', '/tmp/legacy-storage')

    expect(resolveStorageRoot()).toBe(normalize('/tmp/research-agent-storage'))
  })
})
